...
**분석 카드 자동 발송은 KBO·NPB·MLB.** 크롤·분석은 시작 **15분 전**에 끝낸다 —
NPB 18:00→17:45, **KBO 18:30→18:15 (2026-09-03 사용자 결정)**. 그 시각 이후
재판정은 하지 않는다. 종전에는 KBO 에 마감선이 없어 이론상 T-2 에 재판정이
돌 수 있었다 — 그 카드는 정확해도 걸 수가 없다.
⚠️ NPB만 **경량 재판정**(캐시 폼+매치업)을 T-10까지 5분 더 연다. 공시가 T-30이라
15분 창이 너무 좁다는 실측 때문이다. KBO는 공시가 T-60~T-50이라 나누지 않는다.
