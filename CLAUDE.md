...
## 🔒 v1.4 동결 — 변수 대장 완성 (2026-09-05)

🔴 **이 시점부터 리그별 `graded` 50건까지 판정 입력·프롬프트·게이트·
   대장 구성을 전면 동결한다. 예외는 발송 중단급 P0 뿐이다.**

동결 대상: 자료 1~11 의 구성과 내용 · 판정 프롬프트 문구와 규칙 ·
확률 게이트(58%/원정 63%/상한 68%) · 클립(0.32~0.68) · 가치 게이트 ·
트리거 정의(T1~T6) · **변수 대장의 축 구성과 prior 헤더** ·
조정 폭(±3%p·±4%p) · 상한(경기당 5검색·유료 CAP 3).

**표본은 2026-09-05 슬레이트부터 전 종목 일괄 재시작한다.**
사유: 변수 대장 완성. 오늘 하루에 자료7·8 폐지 · 자료9 최근폼 교체 ·
자료10·11 대장 통합 · prior 헤더 신설 · 자료2(뉴스) 경로 복구가 겹쳤다 —
어제까지의 표본과 **같은 시스템이 아니다.** 여기서 끊지 않으면 50건이
서로 다른 재료로 채워진다.
⚠️ `FREEZE_RESTART_IS_FINAL = True` — **이것이 마지막 재시작이다.**
   재료를 바꿀 때마다 미루면 영원히 50건에 도달하지 못한다.

카운터는 일일 요약의 `N/50` 줄에서 본다.
"더 좋아질 것 같다"는 예외가 아니다. 카드가 안 나가거나 틀린 값이 나가는
결함만 예외다.

### 이번 개편 이력 (2026-09-04 ~ 09-05)

| 단계 | 내용 |
|---|---|
| C1 | 자료9 불펜 — 시즌 ERA → 최근 3경기 실점 + 최근 3일 가용성 |
| C2 | 자료7·8 폐지 (시즌 누적은 판정 입력이 아니다) |
| C2' | 자료11 신설 → 연전·원정연전·이동·구장변경·날씨 라벨·선발손 |
| C3 | 플래툰 **보류** (3리그 관문 불통과) |
| C4 | 자료10·11 → **변수 대장** 한 블록 + prior 헤더 |
| C5 | BvP 금지 명문화 + 유입 0건 증명 + 팀별 보정 스텁 |
| C6 | 문서·동결·표본 재시작 |

→ 상세: [docs/SYSTEM_COMPLETE_2026-09-05.md](docs/SYSTEM_COMPLETE_2026-09-05.md)

---

## 🔒 v1.3 동결 (2026-09-03) — v1.4 로 대체됨
