...
- **승률은 경기력만으로.** `p_final = 0.50*p_model + 0.50*p_claude`. **시장 가중치 0.**
  야구 승부는 배당을 조회하지 않는다. 언더오버만 시장 **라인 숫자**를 가져오고 확률은 λ가 답한다.
  judge 프롬프트도 배당을 보지 않는다.
...
- **추천 자격 = 승률 ≥ 58%** (`MIN_WIN_PROB`). 배당 하한 없음. 야구는 라인업 확정 필요.
- **신호등**: 🟢 승률 62%↑ / 🟡 58~62% / 🔴 승률 미달 / ⚪ 확률 미산출
...
- **추천 자격**: 2-소스 룰(①실데이터 ②전문가 ③모델 중 2개 이상) + 승률 하한 +
  신뢰도 연동(판정 '낮음'은 전 마켓 박탈, 축구 승패 단식은 '높음'만).
  야구 승부는 배당을 쓰지 않는다. 언더오버는 라인 숫자만 시장에서 가져온다.
