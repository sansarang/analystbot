...
- **문장 단위 필터**: 딥서치는 "A는 24.2이닝을 소화했다. 다만 3일 분리는 불가능하다"처럼
  실수치와 미확보 단서를 한 값에 섞어 보낸다. 자유서술 필드(note·splits·bullpen·h2h·
  last5_detail)는 미확보 **문장만** 걷어내고 실데이터 문장을 남긴다. 반면 `last5`처럼
  **구간을 단언하는 필드는 전량 폐기** — 시즌 수치만 남아 '최근 성적'인 척 표기되는
  새 오표기를 만들기 때문.
