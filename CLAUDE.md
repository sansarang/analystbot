...
- **야구**: 지난 3경기 폼 + 확정 라인업 + 매치업 Claude. λ 없음. 2-소스 없음.
  ...
  ⚠️ 2026-09-01 개정: **선발투수의 시즌 라인(이닝·ERA·WHIP·BB9)은 표본 보정
  용도로만 허용**한다. 우세를 정하는 데 쓰지 않는다.
  ⚠️ 2026-09-02 개정: **타선·팀의 시즌 지표 금지를 해제**한다. 오늘 타순 9명의
  시즌 타격(PA/AVG/OBP/SLG/OPS/HR/BB/SO)과 팀 가중 OPS는 **정식 근거**다 —
  선발과 달리 표본 보정 전용이 아니다. → 아래 §판정 철학
...
- 입력: 지난 3경기 **원본 박스스코어** + 오늘 타순(번호·포지션)·선발(최근 **5등판**)
  + 72h 뉴스태그 + 라인업 의도(해석) + 선발 시즌 라인(표본 보정 전용)
  + **타순 9명 시즌 타격(정식 근거, 2026-09-02)** + **불펜 ERA·9명(2026-09-02)**.
...
- **선발 시즌 라인 (2026-09-01 개정, 사용자 지시).** 최근 등판 표본이 그 투수를
  대표하는지 판단하는 데만 쓴다. 우세를 정하지 않는다. 근거에는 "표본 보정:"으로
  적는다. 승패(W-L)는 담지 않는다.
  > 실사고 2026-09-01 (NYY @ LAA, 1:7 패): 표본 1경기(6이닝 3실점)를 "안정적"으로
  > 읽고 NYY 66% 추천. 그 투수의 시즌은 ERA 5.40·BB/9 5.4 였고 오늘 3.2이닝
  > 3볼넷 4실점으로 무너졌다. 상대 선발은 시즌 ERA 2.85인데 우리가 본 3경기가
  > 그의 부진 구간이라 "불안"으로 읽혔고, 오늘 7이닝 1실점 0볼넷 8K를 던졌다.
  > **시즌 데이터가 결과를 예측하고 있었는데 판정이 볼 수 없게 되어 있었다.**
- **타선 시즌 라인 (2026-09-02 개정, 사용자 지시).** 오늘 타순 9명의 시즌 타격과
  팀 **타석 가중 OPS**. 선발과 달리 **정식 근거**이며 타선 평가의 주 근거다.
  3경기 팀 득점은 그 위의 변동으로 읽는다. 근거에는 "타선 시즌:"으로 적는다.
  · 타선 격차와 선발 격차가 반대면 **선발이 무겁다** — 선발 하나가 9명 각각보다
    경기에 크게 관여한다.
  · `PA`가 작은 타자의 비율 지표는 믿지 않는다. 수집 실패를 "타선이 약하다"로
    읽지 않는다.
  · 수집: MLB statsapi · KBO 기록실 · NPB npb.jp 팀별 개인 타격. **재현
    (`replay=True`)에서는 붙이지 않는다** — 시즌 값은 언제나 "지금" 값이라
    끝난 경기가 자기 시즌 라인에 들어간다.
