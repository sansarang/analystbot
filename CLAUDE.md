...
**분석 카드 자동 발송은 KBO·NPB만, 매일 17:45 KST.** 라인업 공시는 KBO 1시간 전·
NPB 30분 전(저녁 슬레이트면 둘 다 17:30)이고, 카드는 그 다음 공통으로 보낸다.
MLB·축구는 사용자가 요청할 때만 응답한다. 스케줄러는 데이터를 미리 조사해 캐시한다.
(운영 알림: 크레딧·키 오류·프리페치 리포트)
