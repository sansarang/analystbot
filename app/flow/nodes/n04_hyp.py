...
```
    ...
    # 🔴 [FIX-4a] **파생 마켓은 "걸 대상"이지 질문이 아니다.** ⑪이 종전에
    #    전 마켓에서 max(edge) 를 골라 핸디 edge −46%p 가 best 로 뽑히던
    #    자리라, 마켓을 **하나로 지정**하는 규율은 그대로 둔다.
    # 🔴 [PIPE-1 2026-09-25] **게이트로 가르지 않는다.** 종전은 `동의`에서만
    #    지정했는데, 그것은 R1(시장 역행 금지) 시절 조건이고 09-20 사용자
    #    결정으로 폐지됐다. 그 사이 ⑪은 어느 게이트에서도 픽을 못 냈다:
    #      동의   → p_code≈p_market 이라 승패 edge < 2%p
    #      시장과대·가치의심 → 마켓 미지정이라 구조 픽 자체가 불가
    #    실측 2026-09-25: 야구 17경기가 전건 ⑪에서 보드로 끝났고, 사유가
    #    전부 "가설이 마켓을 지정하지 않았다" 였다.
    #    ⚠️ 두 게이트는 그대로 둔다 — 걸 대상이 없는 자리다:
    #       보드고정   "찾을 것이 없다"
    #       사전값단독 시장이 아직 없어 **파생 가격 자체가 없다**(F-17)
    hyp["market"] = None if gate in (BOARD, PRIOR_ONLY) else "total"
