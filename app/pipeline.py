...
def market_disagreement(pick: dict, settings=None) -> str | None:
    ...
    🔴 [MKT-3 2026-09-09] **한시가 아니라 상시다. `MARKET_AGREE_REQUIRED` 를
       끄지 마라.** 종전 주석은 "재캘리브레이션이 끝나면 꺼라"였고, 그
       재캘리브레이션의 답이 이것이다 (운영 원장 재측정):
         게이트 통과분 6/21 = **28.6%** (P(동전 이하)=0.039)
         보드만 74/133 = 55.6% · 거부권탈락 14/22 = 63.6%
         판정 AUC 0.5122 · 적중 상관 r = **-0.079**   시장 AUC 0.6421
         괴리 상위 20경기 — 판정 35% vs 시장 85%
       ⚠️ "구조적으로 시장을 이길 수 없다"는 지적은 맞다. 다만 우리가 시장을
          이긴다는 증거가 없다 — **갈릴수록 우리가 틀렸다.**
