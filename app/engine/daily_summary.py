...
FREEZE_RESTART_REASON = (
    "최근 폼 전용 대원칙 반영 — 자료7·8 폐지 · 자료9 최근폼 교체 · 자료10·11 신설"
)
#: 🔴 **날짜를 다시 끊지 않았다.** `FREEZE_RESTART_IS_FINAL` 이 막는다.
#   그럴 필요도 없었다 — 2026-09-04 슬레이트의 판정을 새 재료로 **다시
#   돌려 덮었기** 때문에 이 날짜 이후 표본은 전부 같은 재료다.
#   (프리페치 재실행 2026-09-04 17:0x, KBO 5경기 · NPB 5경기)
#   재료를 바꿀 때마다 날짜를 미루면 영원히 50건에 도달하지 못한다.
