...
#: 🔴 [C6 2026-09-05] **전 종목 일괄 재시작.** 오늘 판정 입력이 크게 바뀌었다 —
#   자료7·8 폐지 · 자료9 최근폼 교체 · 자료10·11 을 변수 대장으로 통합 ·
#   prior 헤더 신설 · 자료2(뉴스) 경로 복구. 어제까지의 표본과 같은
#   시스템이 아니다. 여기서 끊지 않으면 50건이 서로 다른 재료로 채워진다.
...
#    다시 시작했다 — 자료12(실력 레이팅)가 판정 입력에 들어갔고, 추천
#    게이트에 시장 동의 조건이 붙었다. 09-06 까지의 판정과 같은 시스템이 아니다.
#    근거: 2026-09-06 620행 분석 — 확신 역전 4중 확인 · 시장 대비 열세의
#    최대 원인 = 실력 축 부재.
#    이전 표본(2026-08-30 ~ 09-06)은 **지우지 않는다** — `pick_ledger` 에
#    `date` 로 남아 있고 v1.3 비교군이다.
