...
UNIT_PATTERNS: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ...
    (r"(\d+(?:\.\d+)?)\s*이닝", "ip",
     ("innings", "starter_ip", "이닝", "최장", "p25", "p50", "p75",
      "다음등판_평균이닝",
      # [2026-09-07] 자료10 리그 선발 기준선. 패턴은 늘리지 않고 **키만** 더한다.
      "평균이닝")),
    ...
    (r"(\d+(?:\.\d+)?)\s*실점", "r",
     ("r", "starter_r", "opp_runs", "runs_allowed_l3", "다음등판_평균실점",
      "실점", "경기당실점",
      # [2026-09-07] 자료1 상대 보정·자료10 리그 선발 기준선.
      #   자료12 가 감시 없이 배포됐던 실수를 되풀이하지 않는다 —
      #   새 숫자는 **같은 커밋에서** 대조 목록에 넣는다.
      "상대평균실점", "평균실점")),
    (r"(\d+(?:\.\d+)?)\s*자책", "er", ("er", "자책")),
    # [2026-09-07] 자료14 분기점 조사 — 상대 타자 수(TBF). 투구수가 DB 에 없어
    #   소모 대리값으로 쓰는 값이라 감시에도 같이 올린다.
    (r"(\d+(?:\.\d+)?)\s*타자", "tbf", ("batters", "상대타자", "평균상대타자")),
    # [자료12] 레이팅 — 1000~1999 만 잡는다(1500 이 리그 평균).
    # 🔴 [FA-1 2026-09-11] 뒤 경계를 `\b` 에서 `(?!\d)` 로. 숫자 뒤에 한국어
    #    조사(`로`·`으로`)가 붙으면 둘 다 **단어문자**라 `\b` 가 성립하지 않아
    #    정수까지만 잡혔다 — "1489.9로" → 1489.
    #    그 탓에 판정이 옳게 인용한 값이 환각으로 찍혔다. 실측 2026-09-11:
    #    `W-FACT-MISMATCH` 8건 전수 확인에서 **8건 모두 오탐**이었고, 경보의
    #    "주장" 값이 전부 `.0` 으로 끝나는 것이 증거였다.
    #    (쉼표·괄호가 뒤에 오면 멀쩡했다 — 그래서 일부만 틀려 더 찾기 어려웠다.)
    #    ⚠️ `(?!\d)` 는 "더 긴 숫자의 일부가 아니다"라는 원래 의도를 그대로
    #       지킨다 — `15095` 에서 `1509` 를 물지 않는다(계약 테스트가 잠근다).
    (r"\b(1\d{3}(?:\.\d+)?)(?!\d)", "elo", ("레이팅",)),
    ...
    (r"(\d+(?:\.\d+)?)\s*득점", "runs",
     ("runs", "runs_l3", "runs_per_game_l3", "우리득점")),
    # [2026-09-07] 자료1 상대 보정의 `배율`.
    #   ⚠️ 캡처 그룹은 **하나만** 둔다 — 추출부가 `m.group(1)` 만 읽으므로
    #      교대(|)로 두 그룹을 쓰면 한쪽 분기에서 None 이 된다.
    (r"배율\s*[:은는]?\s*(\d+(?:\.\d+)?)", "ratio", ("배율",)),
    (r"(\d+(?:\.\d+)?)\s*배(?![율수])", "ratio", ("배율",)),
