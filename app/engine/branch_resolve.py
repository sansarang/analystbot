...
#  🔴 [BRR-1 2026-09-08] **라인업 확정 자체를 못 잡고 있었다.** 실측
#     2026-09-07 다저스 실카드 —
#       "Ohtani의 선발 라인업 복귀 및 타순 배치 확정 여부"
#     `오늘\s*라인업`·`금일\s*라인업` 만 있어 안 걸렸고, `_RECORD_PAT` 의
#     `타순` 에 걸려 기록형이 됐다. 그 뒤 `resolve` 의 타선 분기는 `타선` 만
#     보고 `타순` 은 안 보므로 투수 분기로 떨어져
#     **"질문에서 대상 선발을 특정하지 못했다"** 로 빈손이 됐다.
#     답(라인업 공시 시각·결장 명단)은 이미 `live_status` 가 갖고 있었다.
#     ⚠️ `라인업`·`타순` 만으로 넓히지 않는다 — "타순 상위 3명의 최근 3경기
#        득점력" 같은 **기록형**을 빼앗는다. `확정|복귀|배치` 와 붙을 때만이다.
_LIVE_PAT = (r"구단\s*발표|발표\s*여부|제한\s*여부|오늘\s*라인업|금일\s*라인업|"
             r"선발\s*발표|공시|당일\s*발표|가용\s*여부|출전\s*여부|"
             r"라인업[^,.]{0,20}(?:확정|복귀)|(?:확정|복귀)[^,.]{0,20}라인업|"
             r"타순\s*배치|복귀\s*여부|확정\s*여부")
...
async def resolve(pool, jg: dict, question: str) -> dict:
    ...
    #    **30%(31건)가 투수 질문**이었다. 가로채는 낱말은 `타선` 이 아니라
    #    `부진`·`반등` 이다:
    #      "홈 선발 카라스코 최근 3경기 평균 5.78이닝 3.67실점 **부진**"
    #    사용자가 본 형태(2026-09-07 다저스 실카드):
    #      질문 "Nick Lodolo가 홈 타선을 상대로 5이닝 이상을 소화하며 버텨주는가"
    #      답   "Dodgers 최근 3경기 경기당 6.00득점이 다음 경기에 반등하는가"
    #    ⚠️ 이름만으로 가르지 않는다 — "원정 타선이 Lodolo를 상대로 배율
    #       1.29를 유지하는가" 는 진짜 타선 질문이고, 그걸 빼앗으면 회귀
    #       표본을 잃는다. **주체가 투수이고 동시에 투수의 결과를 물을 때**만
    #       아래 투수 해결사로 내려보낸다.
    #    ⚠️ 주체 판별을 여기 새로 적지 않는다 — `variable_ledger.subject_of`
    #       를 부른다(아래 투수 분기가 이미 쓰던 것이다. 위치만 올렸다).
