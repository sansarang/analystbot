...
**주로 찾을 것 — 팀의 공기**: {situation}.
  구단 공식 계정·비트기자의 현장 속보가 X 에 먼저 뜬다. 기사보다 빠르다.
함께 찾을 것: 선발 변경 · 결장/부상 · 라인업 발표 · 구단 공지.

🔴 **경기 전 것만.** 끝난 경기의 결과·스코어·하이라이트는 빼라 —
   지나간 사건은 오늘의 공기가 아니다.
찾지 말 것: 응원·예상·중계 안내·경기 결과 회고·팬 반응.
...
def should_fire(rss_count: int, lineup_status: str | None,
    ...
    조건: (상황 변수 상시) OR (T-N 라인업 미확정) OR (RSS < 하한)

    🔴 [2026-09-06 사용자 지시] **경기 자체가 아니라 변수를 서치한다.**
       종전 조건 둘은 **라인업 정찰용**이라 MLB·KBO 에서 영영 안 걸렸다 —
       실측: 운영 1회 표식이 NPB 4건뿐이고 MLB·KBO 0건이었다.
         MLB  RSS 21~23건 · 라인업 confirmed(3시간 전 공시) → 둘 다 미달
         KBO  RSS 23~27건 · 판별 시점이 T-90분 밖         → 둘 다 미달
         NPB  라인업 공시가 30분 전이라 T-90분에 미확정    → 걸림
       X 의 값어치는 구단 공식 계정·비트기자의 **현장 속보**(은퇴식·경질설·
       트레이드)이고, 그건 라인업과 무관하게 존재한다.

    🔴 **RSS 0건도 조건에 포함된다.** "재료가 아예 없는 경기"야말로 AI 층이
       필요한 경우인데, `if rss_count:` 같은 판별을 쓰면 0건이 통째로
       빠진다 — 조용한 0 의 전형이다.

    ⚠️ 상시 발동이라도 **경기당 1콜**(`_once_ok`)과 **일일 캡**은 그대로다.
       캡에 걸리면 조용히 0 이 아니라 경고 한 줄을 남긴다.
    """
    s = settings or _cfg()
    if bool(getattr(s, "scout_xsearch_situation", False)):
        return True, "상황 변수 상시 (경기당 1콜·일일 캡 내)"
