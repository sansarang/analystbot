...
_SOCCER_SOURCE = {
    ...
    "J1 리그": "yahoo",
    # 🔴 [ACL-1 2026-09-15] ACL 엘리트. ⚠️ 이 표는 **리그 단위**인데 ACL 은
    #    한 경기 안에 한국·일본·중국·베트남·호주 팀이 섞인다. 다음(한국)을
    #    고른 것은 ACL 기사량이 가장 많아서이고, **일본 홈 경기에서는
    #    야후가 더 낫다.** 팀별 소스 선택은 이 표의 모양으로는 못 한다 —
    #    한계를 알고 남긴다(고치려면 표를 팀 단위로 바꿔야 한다).
    "ACL엘리트": "daum",
}
