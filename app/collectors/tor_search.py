"""[SAT-7] 토르 경유 검색 — 직접 소스가 놓친 것을 DDG 로 보강.

🔴 **[SAT-S4 2026-09-12] 토르는 검색엔진 전용이다.** 콘텐츠 사이트에는
   **해롭다**. 같은 컨테이너에서 같은 시각에 잰 값:

     DuckDuckGo      직접 **불가**(AWS IP 차단) · 토르 200   ← 토르가 살린다
     Transfermarkt   직접 200 · 토르 **202 · 본문 0자**
     PremierInjuries 직접 200 · 44KB    · 토르 **403**

   그래서 층1(구조화된 표: `satellite_soccer._tm_fetch` 등)은 **직접 HTTP** 로
   받고, 토르는 검색 질의에만 쓴다. 여기를 늘리기 전에 위 표를 다시 재라.
