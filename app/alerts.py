...
WATCHDOG_CODES = {
    ...
    # 🔴 [WD-1 2026-09-08] 이 둘은 라벨이 없어 텔레그램에
    #    `🚨 W-SOURCE-DRIFT · 점검 필요` 로 나갔다. 둘 다 중요한 경보다 —
    #    전자는 "파서가 조용히 0을 반환한다"(소스 구조 변경), 후자는
    #    "유료 판정이 캡에 닿았다"(비용). `tests/test_watchdog_labels.py` 가
    #    이제 발생 코드와 이 사전을 **양방향으로** 대조한다.
    "W-SOURCE-DRIFT": "정찰 창인데 한 소스만 조용함 (소스 구조 변경 의심)",
    "W-LLM-PAID": "유료 판정이 일일 상한에 도달",
