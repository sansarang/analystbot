"""8/27 NPB — 결과를 모른다고 가정하고 확정 타순으로 예측한 뒤 채점한다.

KBO `eval_kbo_lineup_blind.py`와 같은 회로:
  - 전적은 `starts_at` 이전 boxscore만.
  - Judge 페이로드에서 점수·종료 상태·기존 판정을 제거.
  - 오늘 등판 선발의 시즌 ERA는 넣지 않는다(누수).
  - 팀 OBP는 시즌 누적이라 1경기가 섞일 수 있다 — 보고에 적는다.

사용:
  PYTHONPATH=. uv run python tools/eval_npb_lineup_blind.py --dump /tmp/npb_827_dump.json
"""
...
def _pitcher_from_order(raw) -> str:
    for item in _order(raw):
        s = str(item)
        if "(投)" in s or s.endswith("투수"):
...
async def main() -> int:
    ...
    from app.collectors import npb_stats
    ...
    games = [g for g in blob["games"] if g.get("home_score") is not None]
    history, today_ev = blob["history"], blob.get("today_events") or []
    print(f"날짜 {args.date} NPB 종료·점수 있는 경기 {len(games)}건")

    settings = get_settings()
    teams = await npb_stats.fetch_team_stats()
    print(f"npb.jp 팀 지표 {len(teams)}/12  league_obp="
          f"{npb_stats.league_baselines(teams).get('obp')}")
    ...
    for g in games:
        ...
        hp = (g.get("home_pitcher") or "").strip() or _pitcher_from_order(ho)
        apn = (g.get("away_pitcher") or "").strip() or _pitcher_from_order(ao)
        res = {"home_pitcher": {"name": hp},
               "away_pitcher": {"name": apn},
               "home_lineup": {"order": ho}, "away_lineup": {"order": ao}}
        npb_stats.merge_into_research(res, {"home": home, "away": away}, teams)
        ...
        jg = {
            "game_id": gid, "sport": "npb", "league": "NPB",
        ...
        print(f"  λ p_home={p_model}"
              + (f"  λ={jg['lam']['home']}/{jg['lam']['away']}" if jg.get("lam") else "")
              + (f"  model_valid={jg['model_valid']}"))
    ...
    verdict = await Judge().judge({
        "date": args.date, "sport": "npb", "games": judge_games,
