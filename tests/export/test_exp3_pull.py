...
def test_덮어쓰지_않는다():
    """🔴 [EXP-4 2026-09-19] 규칙이 바뀌었다 — **번호가 아니라 내용 비교**다.

    종전엔 겹치면 `_r2` 를 붙였는데, 서버도 재실행 때 `_r2` 를 붙인다.
    두 `_r2` 의 뜻이 달라 같은 이름이 다른 내용을 가리켰다(실측으로 잡았다).
    지금은 같으면 건너뛰고 다르면 서버 시각을 붙인다.
    """
    body = _body()
    assert "cmp -s" in body, "내용을 비교하지 않고 덮는다"
    assert 'cp "$TMP" "$dst"' in body, "임시 파일을 거치지 않는다"
