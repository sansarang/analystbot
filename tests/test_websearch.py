...
def test_폐기_건수를_사유별로_돌려준다():
    ...
    assert len(rows) == 1
    assert m == {"받음": 4, "채택": 1, "모름": 1, "부재": 0, "폐기": 2,
...
def test_부재_확인은_근거일자_없이도_통과한다():
    """🔴 실측 2026-09-12 (배선 증명, 강제 주입 2문):

         [websearch] 받음 2 · 채택 0 · 폐기 2 (날짜없음 2)

       두 질문은 "우천 취소 보도가 있는가" · "마무리가 어제 연투했는가" —
       **답이 "없다"인 질문**이다. 부재를 확인한 답에는 인용할 기사가 없으니
       근거일자를 못 적고, 게이트가 전부 버렸다.
       누적 8문 중 3건(37.5%)이 이렇게 사라졌다.

    ⚠️ 근거는 기사가 아니라 **검색한 날**이다. 그래서 시점은 오늘이 된다.
    """
    it = {"번호": 1, "답": "우천 취소·지연 보도는 확인되지 않았다",
          "근거유형": "부재확인"}
    rows, m = WS.parse_answers(_payload(it), TODAY)
    assert len(rows) == 1
    assert rows[0]["시점"] == TODAY
    assert rows[0]["소스유형"] == WS.ABSENCE
    assert m["부재"] == 1 and m["폐기"] == 0


def test_부재_확인임을_행에_드러낸다():
    """🔴 판정이 "기사로 확인된 사실"과 "없더라"를 구분할 수 있어야 한다."""
    it = {"번호": 1, "답": "연투 기록 없음", "근거유형": "부재확인",
          "소스유형": "뉴스"}
    rows, _ = WS.parse_answers(_payload(it), TODAY)
    assert rows[0]["소스유형"] == WS.ABSENCE, "부재확인이 뉴스로 둔갑했다"


def test_기사_주장은_여전히_날짜가_필요하다():
    """🔴 반대 위험 — 이 칸으로 날짜 게이트를 우회하면 SRCH-2 가 막은
    결함(5개월 전 기사)이 그대로 돌아온다."""
    it = {"번호": 1, "답": "구자욱이 가슴뼈 미세골절로 빠졌다", "근거유형": "기사"}
    rows, m = WS.parse_answers(_payload(it), TODAY)
    assert rows == [] and m["폐기_날짜없음"] == 1


def test_근거유형이_없으면_기사로_본다():
    """안전한 쪽으로 — 모르면 날짜를 요구한다."""
    it = {"번호": 1, "답": "무언가 있었다"}
    rows, m = WS.parse_answers(_payload(it), TODAY)
    assert rows == [] and m["폐기_날짜없음"] == 1


def test_부재확인인데_날짜를_적었으면_그_날짜를_쓴다():
    """모델이 굳이 날짜를 적었으면 그것도 게이트를 지난다."""
    it = {"번호": 1, "답": "없다", "근거유형": "부재확인",
          "근거일자": "2026-04-01"}
    rows, m = WS.parse_answers(_payload(it), TODAY)
    assert rows == [] and m["폐기_오래됨"] == 1


def test_프롬프트가_부재_확인_칸을_알려준다():
    p = WS.build_prompt(_jg(), ["q"], TODAY)
    assert "근거유형" in p
    assert "부재확인" in p
...
def test_행에도_같은_게이트를_걸_수_있다():
    """🔴 퍼플렉시티도 **같은 문**을 지난다 — 게이트를 두 벌 만들지 않는다."""
    rows = [{"질문": "", "답": "오늘 일", "소스": "pplx", "소스유형": "뉴스",
             "시점": TODAY, "계정": "", "url": ""},
            {"질문": "", "답": "5개월 전 일", "소스": "pplx", "소스유형": "뉴스",
             "시점": "2026-04-14", "계정": "", "url": ""},
            {"질문": "", "답": "날짜 없음", "소스": "pplx", "소스유형": "뉴스",
             "시점": "", "계정": "", "url": ""}]
    kept, m = WS.gate_rows(rows, TODAY)
    assert [r["답"] for r in kept] == ["오늘 일"]
    assert m["폐기_오래됨"] == 1 and m["폐기_날짜없음"] == 1


def test_parse_answers_가_같은_문을_쓴다():
    """🔴 사본 금지. 두 벌이면 한쪽만 고쳐진다."""
    import inspect

    assert "gate_rows" in inspect.getsource(WS.parse_answers)
