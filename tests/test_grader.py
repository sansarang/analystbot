...
def test_scheduler_jobs_registered():
    ...
    assert set(jobs) == {"prefetch_evening", "prefetch_dawn", "prefetch_asia",
                         ...
                         "pregame_push_1745", "asia_lineup_1700",
                         # [7-5] 하트비트가 있어야 /health가 스케줄러 생존을 안다
                         "heartbeat_2m"}
    assert str(jobs["pregame_push_1745"].trigger) == "cron[hour='17', minute='45']"
    assert "17" in str(jobs["asia_lineup_1700"].trigger)
    assert "40" in str(jobs["asia_lineup_1700"].trigger)
