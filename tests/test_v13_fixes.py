...
async def test_freeze_counter_starts_from_todays_slate():
    ...
    # 🔴 재시작 날짜를 **베끼지 않는다** — `FREEZE_START_DEFAULT` 가 원본이다.
    #    (C6 2026-09-05 변수 대장 → v1.4 2026-09-07 실력 축 복원)
    from app.engine.daily_summary import FREEZE_START_DEFAULT as _FSD

    assert seen["args"][1] == [_FSD, _FSD], \
        "이전 표본이 섞이면 안 된다"
