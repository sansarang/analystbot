...
def test_토르는_보강이지_주력이_아니다():
    ...
    # 팀당 1질의 = 경기당 2질의
    # ⚠️ [SCT-7 2026-09-14] 창(window)으로 세는 방식을 버린다 — 질의 조립이
    #    호출 위로 가고(SCT-4) RSS 통로가 그 사이에 들어오면서(SCT-7) 창이
    #    두 번 깨졌다. 세려던 것은 처음부터 "**함수 전체에서 팀 루프가
    #    한 번**"이다. 그것을 그대로 센다.
    # `_qs` 가 검색 통로(RSS·토르)에 넘어가는 질의 목록이다. 한 번만 쌓으면
    # 팀당 1질의다 — 루프 개수를 세면 다음 채널이 늘 때 또 깨진다.
    assert src.count("_qs.append(") == 1
    assert src.count('for side in ("home", "away")') >= 1
