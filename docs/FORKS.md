...
## F-24 · 배당 이동의 **근거를 무엇으로 찾는가** (2026-09-24 · 딥서치)

**사용자 지시:** "배당변화를 감지하고 근거를 찾는 효과적인 방법을 딥서치해서 찾아라"

### 무엇이 갈렸나

KEY-1(`24b2d69`)로 배선은 이었다 — 근거 붙은 경기 0/14 → 6/14. 그런데
라벨은 `미상`→`자금`이지 **`근거 있음`이 아니다.** 실측:

```
휴스턴@시애틀 · 이 경기 기사 30건
  기사 발행시각   2026-09-15 ~ 2026-09-18      ← 이동은 09-24
  창 30 / 90 / 360분  전부 → 자금 1 · 근거 0
피드 전체 (2026-09-24 KST)
  news_mlb  339건 · 최신 09-23  |  news_npb  315건 · 최신 09-23
  news_kbo  291건 · 최신 **09-21**
```

갈림길은 셋이다 — **① 무엇을 근거의 원천으로 삼는가 ② 창을 얼마로 두는가
③ 이동을 무엇으로 감지하는가.**

### 찾은 자료

#### ① 원천 — 🔴 **RSS 검색 피드는 통신사가 아니라 서고다**

2026-07 실측(48개 질의, Google News RSS):
```
기사 나이 중앙값     약 6.6일
6시간 이내 비율      7.6%
```
> "the median RSS item age was about 6.6 days, and only 7.6% of RSS items
> were six hours old or newer" — [cloro](https://cloro.dev/blog/google-news-rss/)

🔴 **우리 실측(6~9일 전 기사)과 정확히 같은 수치다.** 이것은 우리 코드의
결함이 아니라 **RSS 검색이라는 채널의 성질**이다. 창을 넓히거나 질의를
고쳐도 해결되지 않는다.

권고된 대안은 구조화 엔드포인트다. 우리에게 적용 가능한 것:

| 원천 | 시각 | 비용 | 비고 |
|---|---|---|---|
| **`statsapi.mlb.com/api/v1/transactions`** | 공식·구조화 | **무료·이미 씀** | IL 등재·콜업·DFA. `playerId` 또는 날짜 질의 |
| **probable pitchers / lineups** (statsapi) | 공시 시각 | **무료·이미 씀** | 선발 교체가 MLB 최대 이동원 |
| 배당 자체(다북) | `captured_at` | **이미 있다** | → ③ |
| 구단·리그 공식 발표 | 게시 시각 | 무료 | KBO·NPB 는 statsapi 대응물이 없다 |

> "Pitcher scratches are the most dramatic and exploitable news-driven line
> movements" — [Baseball Bets of the Day](https://baseballbetsoftheday.com/articles/mlb-line-movement-explained/)

#### ② 창 — 인트라데이 이벤트 스터디

```
거시지표 공시 표준창   공시 10분 전 ~ 20분 후 (총 30분)
가격 조정의 80%+       첫 5분 안에 끝난다
```
창 길이는 고정값이 아니라 **CAR 이 유의하지 않아지는 지점까지** 늘리는
자료 기반 결정이다 — [EventStudyTools](https://www.eventstudytools.com/intraday-event-studies)

⚠️ 우리 기본값 `move.window_min: 30` 은 이 표준과 같은 자리다. **창이
   문제가 아니다** — 360분으로 넓혀도 0이었다(위 실측).

#### ③ 감지 — 🔴 **다북 동시성이 노이즈와 신호를 가른다**

```
기본 스팀 감지    60초마다 다북 수집 →
                  5분 창 안에 **3개 이상 북이 같은 방향**으로 움직이면 표시
```
— [DeucesCracked](https://www.deucescracked.com/sports-betting/strategy/line-movement)

한 북만 움직이면 그 북의 사정(리스크 관리·오류)이고, 여러 북이 동시에
움직이면 정보다. 역라인 이동(RLM)은 **대중 베팅 비율과 반대 방향**으로
움직이는 것인데, 비율 자료는 유료라 우리 범위 밖이다.

⚠️ **우리가 실제로 가진 다북 자료 (실측 2026-09-24, 최근 2일):**
```
provider=oddsapinet   북 54  ·  경기 **9**     (totals·spreads 위주)
provider=oddsportal   북  1  ·  경기 28
provider=espn         북  1  ·  경기 31
h2h 계열 경기당 북 — 평균 2.51 · 최소 1 · 최대 13
```
🔴 **다북 감지는 원리상 맞지만 커버리지가 9경기다.** 나머지는 북 1~2개라
   "3개 이상 동시"를 판정할 수 없다.

#### ④ 반대 증거 — 이동이 늘 정보인 것은 아니다

분데스리가 한 시즌 **1Hz** 배당·베팅액으로 첫 골 직전을 본 연구:

> "neither side of the market anticipates goals by significantly adjusting
> their behaviour" — [Winkelmann & Deutscher, arXiv:2505.21275](https://arxiv.org/abs/2505.21275)

🔴 **초당 해상도로도 예측적 이동이 없었다.** "이동에는 반드시 이유가 있다"는
전제를 그대로 믿으면 안 된다는 반대 자료다. 사용자 지시 "이유 미상은 없다"는
**찾아보지도 않고 미상이라 적지 말라**는 뜻으로 읽는다 — 찾아본 뒤 없으면
`자금`이고, 그것이 지금 라벨 체계가 이미 구분하는 바다.

### 무엇을 고를까 — 🔴 **아직 고르지 않았다. 사용자 지시 대기**

자료가 가리키는 순서는 아래와 같으나, 셋 다 **새 수집 경로**라
"코드 추가는 승인 전 금지" 규칙에 걸린다.

| 순위 | 무엇 | 왜 이 순서인가 | 비용 |
|---|---|---|---|
| 1 | statsapi `transactions` + 선발 공시를 근거 원천에 추가 | 유일하게 **공식 시각**이 붙는다. 이미 쓰는 API·무료 | 수집기 1곳 |
| 2 | 다북 동시성으로 `money` 를 다시 나눈다 | 자료가 **이미 DB 에 있다** | 판정부 1곳 |
| 3 | RSS 는 **배경**으로 강등 | 중앙값 6.6일짜리를 ±30분 창에 맞추는 것은 원리상 불가 | 라벨 1개 |

### 안 고른 쪽은 왜

- **창을 넓힌다** — 360분까지 실측했고 0이었다. 6.6일과 30분의 간극은
  창으로 메울 수 없다.
- **질의를 고친다** — 기사 나이 분포는 질의가 아니라 채널의 성질이다.
- **베팅 비율(RLM) 자료를 산다** — 유료 API 금지(사용자 지시).
- **`unobserved` 를 없애고 전부 `money` 로** — "찾아보지 않았다"와
  "찾아봤는데 없다"는 다른 사실이다. 합치면 정보가 준다.

### 🔴 [정정 2026-09-24] F-24 — "RSS 는 서고다"는 **절반만 맞았다**

사용자 지적: "기존에 rss로 모으는거 있지 않았니?" — **있었다.**

F-24 는 Google News **검색** RSS 한 종류만 보고 "RSS 는 원천으로 못 쓴다"고
적었다. 그런데 이 저장소에는 RSS 수집기가 **둘**이고 신선도가 전혀 다르다:

| 무엇 | 질의 | 실측 기사 나이 | 흐름이 읽나 |
|---|---|---|---|
| `crawler/…/news.go` | **리그** Bing 검색(`MLB injury`) | **6~9일 전** | ② 읽는다 |
| `app/collectors/news_rss.py` | **팀별** 구글 뉴스(모국어) | **0.3h ~ 5.6h** | 🔴 **안 읽었다** |

실측 2026-09-24 13:1x KST · KBO 3경기 68건:
```
[home] 0.6h  '8G 19홈런 정체' 허인서가 무조건 신인왕? …
[away] 4.9h  최원태, 혹시 '폰태'로… 하필 지금 **부상 이탈**→어차피 10월 복귀니까
[away] 0.3h  [비하인드]설마 그럴리가… '보스패싱' 논란? …
```

🔴 **차이는 매체가 아니라 질의다.** 팀 이름으로 모국어 질의를 던지면
   당일 기사가 나온다. `news_rss` 머리말이 이미 그렇게 적고 있었다 —
   "영문명으로 던지면 72시간 필터가 전부 걸러낸다. 모국어로 던지면…"

🔴 **그래서 F-24 의 권고 1번(statsapi transactions 수집기 신설)은 보류한다.**
   새 원천을 만들기 전에 **이미 있는 것을 흐름에 잇는 것**이 먼저였다.
   (statsapi `transactions` 도 이미 있다 — `mlb.fetch_transactions`, 위성이
   부른다. 그것마저 새로 만들 뻔했다.)

⚠️ ②의 크롤러 채널은 **치우지 않는다.** 배당 이동과 **시각을 맞춘** 것은
   그쪽뿐이라 "이 이동의 원인"에는 여전히 ②가 맞다. 팀 뉴스는 ②가 방향을
   못 냈을 때의 보조다(`n05._news_rss_dir`).
