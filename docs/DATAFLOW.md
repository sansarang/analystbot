...
### 시장 기준선 — `engine/market_baseline.py` (2026-09-04)

우리 판정과 시장을 나란히 놓고 **나중에 누가 맞았는지 센다.**

| | |
|---|---|
| 계산 | 2-way de-vig. 실측 LAD 1.32 / STL 3.42 → **0.7215** (마진 5.0% 제거) |
| 기준 스냅샷 | `SEND` = 지금 최신(나이 ≤ `market_snapshot_max_age_min`) · `CLOSE` = 시작 직전 마지막 |
| provider | **레지스트리가 원본** — 그 리그 담당 소스만 |
| 카드 | `시장 72% vs 우리 63% (-9.2%p — 시장 이견)`. 값 없으면 **줄 자체 없음** |
| 원장 | `market_baseline_ledger` — 발송 시 upsert, 채점 시 close·hit |
| 채점 | **기존 grade 잡에 이어붙임** (새 잡 없음). `our_hit` 은 `pick_ledger` 복사 |
| 요약 | `🎯 시장 N승M패 · 우리 N승M패 · 이견 k건 중 j적중` |

🔴 **게이트가 아니다.** "시장 동의 → 추천 강등"은 게이트 변경이라 동결
   대상이고, v1.4 승격 후보로만 예약돼 있다. 추천 판정은 기존 게이트 그대로다.
🔴 **`verdict_hash` 가 시장값을 보지 않는다.** 배당은 30분마다 흔들린다 —
   해시에 들어가면 배당 변동만으로 수정 카드가 계속 나간다.
⚠️ 무승부 배당은 수집하지 않는다. 무승부는 채점에서 `void` 로만 본다.
⚠️ 분모: **발송된 경기만** 원장에 행이 생긴다 — 시장과 우리를 같은 경기
   집합에서 비교하기 위해서다.
