...
| D15 | FotMob 경기·팀 매칭 실패 · `norm()` 의 ø·æ 처리 · 개명 팀 | 09-20 매칭 실패 6경기 9칸 · `Brøndby IF` → `brondby if`(치환표 적용 후 실측 OK) | **배포됨** | `2c6daf0` | W2 |
...
| D26 | `fotmob.norm()` 이 **한글을 통째로 지운다** | `norm('박건우')` → `''` (NFKD → ascii ignore). 이름 대조에 그대로 쓰면 한국 선수가 전부 일치한다 | **열림** | W2 |
