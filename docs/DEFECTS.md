...
## 🔴 D54 — 채점이 **카드와 다른 쪽을 채점했다** (2026-09-22 · 고침)

실측 · 운영 원장 · 채점 완료 · `p_code` 있는 **251행**:
...
원인: `predicted_side()` 의 우선순위가 `favored → stored → p_home` 이었다.
정본은 `stored`(= `predicted_side` 칸 = `matchup["승자"]` = **코드가 정한 승자**)다.
`favored` 는 `matchup["우세"]`(레거시 라벨 · '박빙' 포함)이고
`p_home` 은 `jg["p_claude"]`(**은퇴한 옛 Judge 확률**)다.

고침: 순서를 `stored → favored → p_home` 으로. **함수 하나 · 줄 두 개.**
⚠️ `stored` 가 없으면 종전 그대로 내려간다(그 칸은 1,404행 중 242행에만 있다).
⚠️ **소급 재채점을 하지 않았다** — `grade_pending` 이 `graded_at IS NULL` 만
   집으므로 앞으로만 적용된다. 🔴 **따라서 지금까지의 적중률 숫자는 두 규칙이
   섞여 있다.** 숫자를 조용히 갈아치우지 않는다.
⚠️ 기존 계약 `test_채점이_저장된_방향을_읽는다`(ORD-3)가 `# 우세가 우선` 을
   명시로 잠그고 있었다. **규칙 변경**으로 처리하고 근거를 그 계약에 적었다
   (사용자 승인 2026-09-22). 내 변경을 통과시키려 고친 것이 아니다.

## 🔴 D55 — 캘리브레이션이 **은퇴한 옛 Judge 확률**을 잰다 (2026-09-22 · 등록만)

`app/engine/calibration.py:101`:
```sql
SELECT l.p_home, l.favored, l.confidence, l.gate_result, l.sport …
```
`p_home` 은 `jg["p_claude"]` — **옛 Claude Judge** 의 확률이다. 지금 확률은
`p_code` 이고 그 칸은 **읽히지 않는다.**

같은 표본 251행(채점 완료 · `p_code` 있음)에서 셋을 나란히 쟀다:

```
p_code     n=251  AUC **0.6220** [0.553, 0.691]  브라이어 0.2397   ← 유의(0.5 밖)
p_market   n=251  AUC  0.6167  [0.547, 0.686]  브라이어 0.2397   ← 유의
p_home     n=156  AUC  0.5007  [0.410, 0.592]  브라이어 0.2497   ← 0.5 포함
```

🔴 **CLAUDE.md 의 "판정 확률 AUC 0.5122 · 브라이어 0.2537 · 50%로 찍는 것보다
   나쁘다"는 `p_home`(옛 Judge) 의 값이다.** 실측 0.5007 로 지금도 재현된다.
   **지금 확률(`p_code`)은 AUC 0.622 로 유의하다.**

⚠️ **과장하지 않는다.** `p_code`(0.622)와 `p_market`(0.617)의 차이는
   **유의하지 않다**(CI 가 크게 겹친다). 즉 "우리 확률이 정보가 있다"는 말은
   설 수 있지만 "시장을 이긴다"는 말은 **아직 설 수 없다**. 표본도 251행이고
   CLAUDE.md 의 178경기와 다른 표본이다.
...
## D57 — 흐름이 **축구만 27%** 에서 멈춘다 (2026-09-22 · 등록만)

실측 (`analysis_runs ⋈ games`):
```
종목     경기   가설(n04)   판정(n06)   가치(n11)
mlb       64      56          56          40
npb       23      23          23           8
kbo       20      20          20           4
soccer    45    **12**        12           2     ← 33경기가 ③게이트에서 멈춘다
```
야구 3종목은 ③→④ 통과율이 100%·100%·100% 인데 **축구만 27%** 다.

⚠️ **원인을 아직 안 쟀다.** 후보:
   · `n03_gate` 가 축구에서 `보드고정`/`사전값단독`을 많이 낸다(시장 부재)
   · 축구 사전값(`team_elo`)이 K리그1 에서 None 이다(기존 기록:
     "K리그1 은 elo None 이라 모델 무효")
   · 3-way(무승부) 처리가 게이트에서 다르게 걸린다

🔴 **세지 않고 고치지 않는다.** 사용자 지시("페이블식 흐름은 모든 스포츠에
   적용되어야 한다")를 지키려면 이 27% 가 **의도인지 결함인지** 먼저 갈라야 한다.

---

## D58 — UCL·UEL 은 **검색어도 로케일도 없다** (2026-09-22 · 등록만)

LOC-3 으로 리그앙·에레디비시·덴마크를 고치면서 **함께 드러난 자리**다.

```
league_labels()  UCL → 'ucl' · UEL → 'uel'     ← 리그는 실재한다
SEARCH_TERMS     ucl 없음 · uel 없음
locales          ucl None · uel None
```

`rss_hits` 는 둘 중 하나만 없어도 **빈손으로 즉시 반환**한다. 즉 유럽
대항전은 기사 검색이 나간 적이 없다.

🔴 **갈림길이라 고치지 않았다.** UCL 은 한 경기의 두 팀이 서로 다른 나라다 —
어느 언어로 물을지가 갈린다:

| 갈래 | 내용 |
|---|---|
| (가) 영어 단일 | `{hl: en, gl: GB}`. 단순하지만 현지 1차 보도를 놓친다 |
| (나) 팀 국적별 | 팀마다 다른 로케일. 회수는 높지만 표가 팀 단위가 된다 |
| (다) 양쪽 다 | 질의 2배. §3 예산과 부딪힌다 |

⚠️ ACL 은 종전에 `{hl: en, gl: SG}` 로 정해져 있지만 **그 근거가 문서에
   없다.** 같은 갈림길을 누군가 조용히 (가)로 고른 흔적이다.

**2갈래 이상이므로 딥서치 대상이다**(CLAUDE.md 규약) — 지시 없이 고르지
않는다. `tests/test_loc3_locales.py::test_UCL_UEL_은_아직_없다는_사실을_잠근다`
가 "모르는 채 빠진 것이 아니라 알고 뺀 것"임을 잠근다.

---

## D59 · 끝난 경기의 `status` 가 `final` 로 안 바뀐다 (2026-09-23 · 등록만)

**실측** (SIDE-3 원장 복원 중에 드러났다):
```
시작한 지 6시간 넘었는데 status <> 'final'   kbo 23 · soccer 20 · mlb 14 · npb 6
점수는 적재됐는데 status 만 안 바뀐 경기      12건 (30일)
```

🔴 **채점은 `status='final'` 을 본다**(`learning.decisions._PENDING_SQL`).
   그래서 이 경기들은 원장에 남아 있어도 **영영 채점되지 않는다.**

⚠️ **내가 처음에 "채점이 막혔다"고 과장했다.** 실제로는 `decision_ledger`
   `flow_v14` 748행이 **09-22·09-23 이틀치뿐**이고 대부분 아직 안 끝난
   경기다(오늘 KBO 는 18:30 시작). 채점 8건은 그것이 주된 이유다.
   위 63건은 그와 **별개의 더 작은 결함**이다.

⚠️ 점수 적재와 상태 전이가 **한 곳이 아니다** — 점수만 들어오고 상태가 안
   따라오는 12건이 그 증거다. 어디서 갈리는지는 아직 안 쟀다.

**범위 밖이라 등록만 한다.** 고치려면 결과 적재 경로를 재야 하고, 그건
지시받은 작업이 아니다.

---

## D60 · 한 경기·한 시각에 **서로 반대인 두 벌**의 배당이 들어온다 (2026-09-23)

**실측** (MOV-C 실데이터 시험 중 드러났다):
```
g1774 롯데@한화  odds_snapshots
  15:13:42.633720  oddsportal-avg  Hanwha 1.65   snap_tag=open_proxy
  15:13:42.643199  oddsportal-avg  Lotte  2.20   snap_tag=open_proxy
  15:13:42.688453  oddsportal-avg  Hanwha 2.10   snap_tag=(없음)
  15:13:42.693263  oddsportal-avg  Lotte  1.74   snap_tag=(없음)
```
🔴 **둘 다 마진이 정상이다**(1.061 · 1.051). 한 벌은 한화 우세, 다른 벌은
   롯데 우세 — **서로 반대**다. 북도 하나(`oddsportal-avg`)고 provider 도 하나다.

⚠️ `snap_tag` 로는 못 가린다. g1774 의 태그 없는 행만 봐도 46분에 124행
   (분당 2.7행)이라 **두 벌이 모두 태그 없이** 들어온다. 실측:
```
증분 자기상관 ρ₁   전체    kbo −0.5849 · npb −0.5751 · mlb +0.0001
                  태그없음만 kbo −0.5847 · npb −0.5807 · mlb +0.0011
```

**이것이 "배당 이동 노이즈"의 진짜 정체다.** 종전에 내가 ODD-S 에서 찾은
"불가능한 배당"(`1/홈+1/원정 < 1`, kbo 145행)은 **두 벌을 엇갈려 짝지은
결과**였다 — 증상이지 원인이 아니다. ODD-S 게이트는 그대로 쓸모가 있지만
(엇갈린 짝을 버린다) 뿌리는 여기다.

**가짜 이동의 크기** — 아무 쪽이나 고르면 다음 분에 반대 벌이 뽑힌다:
```
g1774  고르고 쓸 때   자금 7구간 · 설명 못한 폭 84.14%p  (±12.02%p 반복)
       모호분 버릴 때  근거 있음 1구간 · 설명 못한 폭 0.0%p
```

**흐름 쪽은 막았다** — `n02_market._sets` 가 한 쪽에 값이 둘 이상인 분을
**버린다**(고르지 않는다). 계약 `test_한_분에_두_벌이_오면_버린다`.

🔴 **수집기는 안 고쳤다 — 범위 밖이다.** 어디서 두 벌이 생기는지(같은 경기의
   다른 경기를 매칭했는지, 페이지에 두 줄이 있는지, `tag_open` 소급 적재가
   `captured_at` 을 지금으로 찍는지)는 `app/collectors/odds_free.py` 를 재야
   알 수 있고, 그건 지시받은 작업이 아니다.

---

## D61 · 배선 전수 재검증 2026-09-24 — **끊긴 자리 셋 · 반쪽 하나**

사용자 2026-09-24: "우선 배선부터 재검증해라..할때마다 배선안되있으면
예측도 잘되겠니?"

**재는 법**: 정적 스캔(고아 함수)은 모듈 내부 호출을 못 봐 오탐투성이라
버렸다. 대신 **운영이 실제로 남긴 `analysis_runs`** 를 읽었다 —
경기당 최신 스냅샷 1회.

```
최근 14시간 · 경기 21 (mlb 16 · npb 2 · kbo 3)
KEY-1 배포(01:39Z) 이후 · 경기 5      ← 14시간 창은 구코드가 섞여 있다
```

### ① 13노드 사슬은 ⑪까지 온전하다

```
n01 21 · n02 21 · n03 21 · n04 21 · n05 21 · n06 21
n07 19 · n08 19 · n09 19 · n10_rejudge 19 · n11 19 · n12 0 · n13 0
재판정(rerun) 3경기에서 ⑤~⑨ 다시 돌았다 — 배선 살아 있다
멈춤: n11_no_value 19 · n03_freeze 5 · n06_unknown 2
```
⑫⑬ 0 은 **끊김이 아니라 판정 결과**다 — 19경기 전부 ⑪에서 가치가 없었다.

### ② 🔴 `play_load` — 운영에서 **아무도 안 넣는다**

```
n05_evidence.py:854   got = (ctx.inject or {}).get("play_load")
inject 하는 곳        tools/backtest_sep.py:175   ← **여기 하나뿐**
운영 실측             5/5 "출전 기록이 없다 — 미실행"
있는 자료             batter_appearances  mlb 9,404 · npb 5,107 · kbo 4,514
```
🔴 LOAD-1(`8cd00dc`)이 만든 `app/flow/load.py` 는 **백테스트에서만** 돈다.
   CLAUDE.md §배선 규칙의 전형이다 — "만든 것은 반드시 부르는 곳이 있어야 한다."

### ③ 🔴 `weather` — 수집 경로가 **아예 없다**

```
21/21 "이 변수는 아직 수집 경로가 없다 — 미실행"
```
변수표(`rules.yaml`)에는 있고 ⑤에 분기가 없다. **만든 적이 없는데 칸만 있다.**

### ④ 🔴 `news_injury` — 칸은 이어졌는데 **창이 구조적으로 안 맞는다**

KEY-1(`24b2d69`) 뒤 ②가 `news_dir` 을 내기 시작했다. 그런데 값이 늘 0 이다:
```
배포 후 5경기 · news_dir = {"home": 0, "away": 0, "basis": ""}
attribution.news_dir  abs(기사시각 − **지금**) > 30분 이면 버린다
RSS 기사 나이         중앙값 6.6일 · 6시간 이내 7.6%   (→ FORKS F-24)
```
🔴 "지금 ±30분"과 "중앙값 6.6일"은 겹칠 수 없다. **창이 아니라 원천 문제**다.
⚠️ F-19("기사 신선도 창이 두 벌이다 · 결정 대기")가 가리키던 바로 그 자리다.

### ⑤ ⚠️ `park_factor` — 값은 오는데 **판정에 닿지 않는다**

```
21경기 중 19 정상 · 방향 있음 **0**
n05_evidence.py:817  _row(var, [...], source=...)   ← direction 인자가 없다
n07_adjust._direction_of({}, side) → 0
실제 값             'Baltimore Orioles 0.950' · 'Detroit Tigers 0.895'
```
카드에 적히기는 하나 확률을 **한 번도 움직이지 않는다.**

### ⑥ ✅ 끊기지 않은 것 — 함께 적는다

```
bullpen_3d         5/5 정상 · db:pitcher_appearances
travel_backtoback  5/5 정상 · 방향 4/5   ← LOAD-1 이어졌다
starter_recent3    2/2 · lineup_out 2/2
설정 키            load.b2b_window_h = 26 · flow.load.b2b_window_h = MISSING
                   (접두사 두 번 붙는 종전 결함 없음)
sport_code         games.sport = mlb·kbo·npb·soccer 와 일치 · KEY-1 이 안 깼다
흐름 진입점        scheduler.py:360 → bridge.run_today → run_slate
```

⚠️ **이 보고 도중 내가 두 번 틀렸다.** (a) `dev` 를 방향으로 읽어
   `travel_backtoback` 이 죽었다고 봤다 — `dev` 는 상수(`_DEV_FULL()`)고
   방향은 `home`/`away` 칸이다. (b) ⑩의 이름을 `n10_guard` 로 적어 도달 0 으로
   봤다 — 실제 이름은 `n10_rejudge` 이고 19경기 돌고 있다.
   **지표를 고치기 전 숫자는 믿지 마라.**

### 고치는 순서 — 🔴 **지시 대기. 손대지 않았다**

| | 무엇 | 크기 |
|---|---|---|
| 1 | `play_load` 를 운영 경로에서 주입 | 수집 1곳 배선 |
